<?php
function makeheader () {
	echo("<!doctype html><html><head><meta charset=\"UTF-8\" /><title>$page</title></head>
	<body><div id=\"wrapper\">");
}
...
function makepage ($page, $edit) {
	...
	if(file_exists($path)){
		...
		echo("<div id=\"pagecontent\"><p>Page does not exist.</p></div>");
		$edit = true;
	}

	$content = tsukimark2($rawcontent);

	echo("<div id=\"pagecontent\"><p>$content</p></div>");
...
function makeform ($content) {
	echo('<form method="post" name="post"><textarea name="content" rows="5" cols="40">' . $content . '</textarea>
...
function makefooter ($page, $edit) {
	...
	echo('<div class="toolbar">
		[<a href="?page=MainPage">Home</a>]
		[<a href="?page=' . $page  . ($edit == true ? '&mode=view' : '&mode=edit') . '">' . ($edit == true ? 'View' : 'Edit') . '</a>]
	</div>
	</div></body></html>');
}

function tsukimark2 ($content) {
	$textAr = explode("\n", $content);
	$content = '';
